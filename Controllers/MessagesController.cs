...
namespace DatingAppCore3.API.Controllers
{
    ...
    public class MessagesController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateMessage(int userId ,[FromBody] MessageForCreationDto messageForCreationDto)
        {
            ...
            if (recipient == null)
                return BadRequest("Could not find result");
        ...
        public async Task<IActionResult> DeleteMessage(int id, int userId)
        {
            ...
            if (messageFromRepo.RecipientId == userId)
                messageFromRepo.Deleted = true;

            if (messageFromRepo.SenderDeleted && messageFromRepo.Deleted)
                _repo.Delete(messageFromRepo);

            if (await _repo.SaveAll())
                return NoContent();
            throw new Exception("Error in deleting message");
